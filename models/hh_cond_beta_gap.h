...
#ifndef HH_COND_BETA_GAP_H
#define HH_COND_BETA_GAP_H
 ...
 */
extern "C" int
hh_cond_beta_gap_dynamics( double, const double*, double*, void* );

/* BeginDocumentation
Name: hh_cond_beta_gap - Hodgkin-Huxley neuron model with gap-junction support.

Description:

 hh_cond_beta_gap is an implementation of a spiking neuron using the
 Hodgkin-Huxley formalism that also supports gap junctions.
...
SeeAlso: hh_psc_alpha, hh_cond_exp_traub, gap_junction, iaf_cond_beta
*/
class hh_cond_beta_gap : public Archiving_Node
{
...
public:
  ...
  hh_cond_beta_gap();
  hh_cond_beta_gap( const hh_cond_beta_gap& );
  ~hh_cond_beta_gap();
...
private:
  ...
  friend int hh_cond_beta_gap_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friend to access the State_ class/member
  friend class RecordablesMap< hh_cond_beta_gap >;
  friend class UniversalDataLogger< hh_cond_beta_gap >;
...
private:
  ...
  struct Buffers_
  {
    Buffers_( hh_cond_beta_gap& ); //!<Sets buffer pointers to 0
    //! Sets buffer pointers to 0
    Buffers_( const Buffers_&, hh_cond_beta_gap& );

    //! Logger for all analog data
    UniversalDataLogger< hh_cond_beta_gap > logger_;
  ...
  static RecordablesMap< hh_cond_beta_gap > recordablesMap_;
};

inline void
hh_cond_beta_gap::update( Time const& origin, const long from, const long to )
{
  update_( origin, from, to, false );
}

inline bool
hh_cond_beta_gap::wfr_update( Time const& origin,
  const long from,
  const long to )
{
...
inline port
hh_cond_beta_gap::send_test_event( Node& target,
  rport receptor_type,
  synindex,
  bool )
{
...
inline port
hh_cond_beta_gap::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
hh_cond_beta_gap::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
hh_cond_beta_gap::handles_test_event( DataLoggingRequest& dlr,
  rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
hh_cond_beta_gap::handles_test_event( GapJunctionEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
hh_cond_beta_gap::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
hh_cond_beta_gap::set_status( const DictionaryDatum& d )
{
