/*
 *  aeif_cond_beta_multisynapse_extg.cpp
 *
...
template <>
void
DynamicRecordablesMap< aeif_cond_beta_multisynapse_extg >::create( aeif_cond_beta_multisynapse_extg& host )
{
  // use standard names wherever you can for consistency!
  insert( names::V_m, host.get_data_access_functor( aeif_cond_beta_multisynapse_extg::State_::V_M ) );

  insert( names::w, host.get_data_access_functor( aeif_cond_beta_multisynapse_extg::State_::W ) );

  host.insert_conductance_recordables();
}

Name
aeif_cond_beta_multisynapse_extg::get_g_receptor_name( size_t receptor )
{
...
void
aeif_cond_beta_multisynapse_extg::insert_conductance_recordables( size_t first )
{
  for ( size_t receptor = first; receptor < P_.E_rev.size(); ++receptor )
  {
    size_t elem = aeif_cond_beta_multisynapse_extg::State_::G
      + receptor * aeif_cond_beta_multisynapse_extg::State_::NUM_STATE_ELEMENTS_PER_RECEPTOR;
    recordablesMap_.insert( get_g_receptor_name( receptor ), this->get_data_access_functor( elem ) );
  }
}

DataAccessFunctor< aeif_cond_beta_multisynapse_extg >
aeif_cond_beta_multisynapse_extg::get_data_access_functor( size_t elem )
{
  return DataAccessFunctor< aeif_cond_beta_multisynapse_extg >( *this, elem );
}
...
extern "C" int
aeif_cond_beta_multisynapse_extg_dynamics( double, const double y[], double f[], void* pnode )
{
  // y[] is the state vector supplied by the integrator,
  // not the state vector in the node, node.S_.y[].

  typedef nest::aeif_cond_beta_multisynapse_extg::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aeif_cond_beta_multisynapse_extg& node = *( reinterpret_cast< nest::aeif_cond_beta_multisynapse_extg* >( pnode ) );
...
aeif_cond_beta_multisynapse_extg::Parameters_::Parameters_()
  : V_peak_( 0.0 )       // mV
  , V_reset_( -60.0 )    // mV
  , t_ref_( 0.0 )        // ms
  , g_L( 30.0 )          // nS
...
aeif_cond_beta_multisynapse_extg::State_::State_( const Parameters_& p )
...
aeif_cond_beta_multisynapse_extg::State_::State_( const State_& s )
  : r_( s.r_ )
{
  y_ = s.y_;
}

aeif_cond_beta_multisynapse_extg::State_& aeif_cond_beta_multisynapse_extg::State_::operator=( const State_& s )
{
...
void
aeif_cond_beta_multisynapse_extg::Parameters_::get( DictionaryDatum& d ) const
{
...
void
aeif_cond_beta_multisynapse_extg::Parameters_::set( const DictionaryDatum& d, Node* node )
{
  ...
  bool g_ext_flag = updateValue< std::vector< double > >( d, names::g_ext, g_ext );
  ...
  { // receptor arrays have been modified
    if ( ( E_rev.size() != old_n_receptors || tau_rise.size() != old_n_receptors
           || g_ext.size() != old_n_receptors || tau_decay.size() != old_n_receptors )
         && ( not Erev_flag || not g_ext_flag || not taur_flag || not taud_flag ) )
    {
      throw BadProperty(
    ...
    {
      throw BadProperty(
        "The reversal potential, external conductance, synaptic rise time and "
        "synaptic decay time arrays must have the same size." );
    }
...
void
aeif_cond_beta_multisynapse_extg::State_::get( DictionaryDatum& d ) const
{
...
void
aeif_cond_beta_multisynapse_extg::State_::set( const DictionaryDatum& d, Node* node )
{
  updateValueParam< double >( d, names::V_m, y_[ V_M ], node );
  updateValueParam< double >( d, names::w, y_[ W ], node );
}

aeif_cond_beta_multisynapse_extg::Buffers_::Buffers_( aeif_cond_beta_multisynapse_extg& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
aeif_cond_beta_multisynapse_extg::Buffers_::Buffers_( const Buffers_& b, aeif_cond_beta_multisynapse_extg& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
aeif_cond_beta_multisynapse_extg::aeif_cond_beta_multisynapse_extg()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
aeif_cond_beta_multisynapse_extg::aeif_cond_beta_multisynapse_extg( const aeif_cond_beta_multisynapse_extg& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
aeif_cond_beta_multisynapse_extg::init_state_( const Node& proto )
{
  const aeif_cond_beta_multisynapse_extg& pr = downcast< aeif_cond_beta_multisynapse_extg >( proto );
  S_ = pr.S_;
}

void
aeif_cond_beta_multisynapse_extg::init_buffers_()
{
  ...
  B_.sys_.function = aeif_cond_beta_multisynapse_extg_dynamics;
...
void
aeif_cond_beta_multisynapse_extg::calibrate()
{
 ...
 * ---------------------------------------------------------------- */
void
aeif_cond_beta_multisynapse_extg::update( Time const& origin, const long from, const long to )
{
...
port
aeif_cond_beta_multisynapse_extg::handles_test_event( SpikeEvent&, rport receptor_type )
{
...
void
aeif_cond_beta_multisynapse_extg::handle( SpikeEvent& e )
{
  if ( e.get_weight() < 0 )
  {
    throw BadProperty(
...
void
aeif_cond_beta_multisynapse_extg::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
aeif_cond_beta_multisynapse_extg::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}

void
aeif_cond_beta_multisynapse_extg::set_status( const DictionaryDatum& d )
{
