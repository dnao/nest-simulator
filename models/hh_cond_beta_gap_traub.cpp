...
nest::RecordablesMap< nest::hh_cond_beta_gap_traub >
  nest::hh_cond_beta_gap_traub::recordablesMap_;
  ...
  void* pnode )
{
  ...
  assert( pnode );
  const nest::hh_cond_beta_gap_traub& node =
    *( reinterpret_cast< nest::hh_cond_beta_gap_traub* >( pnode ) );
  ...
  //Synaptic Currents
  const double& I_ex = ( y[ S::G_EXC ] + node.P_.g_ext_ex ) * ( y[ S::V_M ] - node.P_.E_ex );
  const double& I_in = ( y[ S::G_INH ] + node.P_.g_ext_in ) * ( y[ S::V_M ] - node.P_.E_in );
...
void
nest::hh_cond_beta_gap_traub::State_::set( const DictionaryDatum& d )
{
...
nest::hh_cond_beta_gap_traub::Buffers_::Buffers_( const Buffers_&,
  hh_cond_beta_gap_traub& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
double
nest::hh_cond_beta_gap_traub::get_normalisation_factor( double tau_rise,
  double tau_decay )
{
...
nest::hh_cond_beta_gap_traub::calibrate()
{
  ...
  V_.PSConInit_E_ =
    nest::hh_cond_beta_gap_traub::get_normalisation_factor( P_.tau_Erise,
      P_.tau_Edecay);
  V_.PSConInit_I_ =
    nest::hh_cond_beta_gap_traub::get_normalisation_factor( P_.tau_Irise,
      P_.tau_Idecay);
  ...
  const bool called_from_wfr_update )
{
  ...
  for ( long lag = from; lag < to; ++lag )
  {
    ...
    if ( called_from_wfr_update )
    {
      ...
      {
        hh_cond_beta_gap_traub_dynamics(
