...
nest::RecordablesMap< nest::aiaf_cond_beta > nest::aiaf_cond_beta::recordablesMap_;
...
template <>
void
RecordablesMap< aiaf_cond_beta >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m, &aiaf_cond_beta::get_y_elem_< aiaf_cond_beta::State_::V_M > );
  insert_( names::g_ex, &aiaf_cond_beta::get_y_elem_< aiaf_cond_beta::State_::G_EXC > );
  insert_( names::g_in, &aiaf_cond_beta::get_y_elem_< aiaf_cond_beta::State_::G_INH > );

  insert_( names::t_ref_remaining, &aiaf_cond_beta::get_r_ );
}
}
...
extern "C" inline int
nest::aiaf_cond_beta_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef nest::aiaf_cond_beta::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::aiaf_cond_beta& node = *( reinterpret_cast< nest::aiaf_cond_beta* >( pnode ) );
  ...
  const double I_K = y[ S::G_K ] * ( V - node.P_.E_K );
  const double I_leak = node.P_.g_L * ( V - node.P_.E_L );

  // dV_m/dt
  f[ 0 ] = ( -I_leak - I_syn_exc - I_syn_inh - I_K + node.B_.I_stim_ + node.P_.I_e ) / node.P_.C_m;
...
nest::aiaf_cond_beta::Parameters_::Parameters_()
  : V_th( -55.0 )       // mV
  , V_reset( -60.0 )    // mV
  , t_ref( 2.0 )        // ms
  , g_L( 16.6667 )      // nS
...
nest::aiaf_cond_beta::State_::State_( const Parameters_& p )
  : r( 0 )
{
...
nest::aiaf_cond_beta::State_::State_( const State_& s )
  : r( s.r )
{
...
nest::aiaf_cond_beta::State_& nest::aiaf_cond_beta::State_::operator=( const State_& s )
{
  r = s.r;
  for ( size_t i = 0; i < STATE_VEC_SIZE; ++i )
  {
...
nest::aiaf_cond_beta::Buffers_::Buffers_( aiaf_cond_beta& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::aiaf_cond_beta::Buffers_::Buffers_( const Buffers_&, aiaf_cond_beta& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
void
nest::aiaf_cond_beta::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::aiaf_cond_beta::Parameters_::set( const DictionaryDatum& d, Node* node )
{
...
void
nest::aiaf_cond_beta::State_::get( DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::g_k, y[ G_K ] );
}

void
nest::aiaf_cond_beta::State_::set( const DictionaryDatum& d, const Parameters_&, Node* node )
{
  ...
  updateValueParam< double >( d, names::g_k, y[ G_K ], node );
}
...
nest::aiaf_cond_beta::aiaf_cond_beta()
  : ArchivingNode()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::aiaf_cond_beta::aiaf_cond_beta( const aiaf_cond_beta& n )
  : ArchivingNode( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::aiaf_cond_beta::init_buffers_()
{
  ...
  B_.sys_.function = aiaf_cond_beta_dynamics;
...
double
nest::aiaf_cond_beta::get_normalisation_factor( double tau_rise, double tau_decay )
{
  return nest::beta_normalization_factor( tau_rise, tau_decay );
}

void
nest::aiaf_cond_beta::calibrate()
{
  // ensures initialization in case mm connected after Simulate
  B_.logger_.init();

  V_.PSConInit_E = nest::aiaf_cond_beta::get_normalisation_factor( P_.tau_rise_ex, P_.tau_decay_ex );
  V_.PSConInit_I = nest::aiaf_cond_beta::get_normalisation_factor( P_.tau_rise_in, P_.tau_decay_in );
...
void
nest::aiaf_cond_beta::update( Time const& origin, const long from, const long to )
{
...
void
nest::aiaf_cond_beta::handle( SpikeEvent& e )
{
...
void
nest::aiaf_cond_beta::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
nest::aiaf_cond_beta::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
