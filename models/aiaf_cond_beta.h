 ...
 */
extern "C" int aiaf_cond_beta_dynamics( double, const double*, double*, void* );
...
aiaf_cond_beta is an implementation of a spiking neuron using IAF dynamics with
conductance-based synapses and adaptation using potassiam current.
Incoming spike events induce a postsynaptic change
...
class aiaf_cond_beta : public ArchivingNode
{
...
public:
  aiaf_cond_beta();
  aiaf_cond_beta( const aiaf_cond_beta& );
  ~aiaf_cond_beta();
...
private:
  ...
  friend int aiaf_cond_beta_dynamics( double, const double*, double*, void* );

  // The next two classes need to be friends to access the State_ class/member
  friend class RecordablesMap< aiaf_cond_beta >;
  friend class UniversalDataLogger< aiaf_cond_beta >;
...
private:
  ...
  struct Buffers_
  {
    Buffers_( aiaf_cond_beta& );                  //!< Sets buffer pointers to 0
    Buffers_( const Buffers_&, aiaf_cond_beta& ); //!< Sets buffer pointers to 0

    //! Logger for all analog data
    UniversalDataLogger< aiaf_cond_beta > logger_;
  ...
  static RecordablesMap< aiaf_cond_beta > recordablesMap_;
};
...
inline port
aiaf_cond_beta::send_test_event( Node& target, rport receptor_type, synindex, bool )
{
  SpikeEvent e;
  e.set_sender( *this );
...
inline port
aiaf_cond_beta::handles_test_event( SpikeEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline port
aiaf_cond_beta::handles_test_event( CurrentEvent&, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline port
aiaf_cond_beta::handles_test_event( DataLoggingRequest& dlr, rport receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
aiaf_cond_beta::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d );
...
inline void
aiaf_cond_beta::set_status( const DictionaryDatum& d )
{
