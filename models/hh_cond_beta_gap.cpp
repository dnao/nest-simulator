...
nest::RecordablesMap< nest::hh_cond_beta_gap >
  nest::hh_cond_beta_gap::recordablesMap_;
...
template <>
void
RecordablesMap< hh_cond_beta_gap >::create()
{
  // use standard names whereever you can for consistency!
  insert_( names::V_m,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::V_M > );
  insert_( names::Act_m,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::HH_M > );
  insert_( names::Act_h,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::HH_H > );
  insert_( names::Inact_n,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::HH_N > );
  insert_( names::Inact_p,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::HH_P > );
  insert_( names::g_ex,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::G_EXC > );
  insert_( names::g_in,
    &hh_cond_beta_gap::get_y_elem_< hh_cond_beta_gap::State_::G_INH > );
}

extern "C" int
hh_cond_beta_gap_dynamics( double time,
  const double y[],
  double f[],
  void* pnode )
{
  // a shorthand
  typedef nest::hh_cond_beta_gap::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::hh_cond_beta_gap& node =
    *( reinterpret_cast< nest::hh_cond_beta_gap* >( pnode ) );
  ...
  const double& I_ex = ( y[ S::G_EXC ] + node.P_.g_ext_ex ) * ( V - node.P_.E_ex );
  const double& I_in = ( y[ S::G_INH ] + node.P_.g_ext_in ) * ( V - node.P_.E_in );
  ...
  f[ S::DG_EXC ] = -y[ S::DG_EXC ] / node.P_.tau_Edecay;
  f[ S::G_EXC ] = y[ S::DG_EXC ] - ( y[ S::G_EXC ] / node.P_.tau_Erise );
...
nest::hh_cond_beta_gap::Parameters_::Parameters_()
  : t_ref_( 2.0 )      // ms
  , g_Na( 4500. )      // nS
  , g_Kv1( 9.0 )       // nS
  , g_Kv3( 9000.0 )    // nS
...
nest::hh_cond_beta_gap::State_::State_( const Parameters_& )
  : r_( 0 )
{
...
nest::hh_cond_beta_gap::State_::State_( const State_& s )
  : r_( s.r_ )
{
...
nest::hh_cond_beta_gap::State_& nest::hh_cond_beta_gap::State_::operator=(
  const State_& s )
{
...
void
nest::hh_cond_beta_gap::Parameters_::get( DictionaryDatum& d ) const
{
...
void
nest::hh_cond_beta_gap::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::hh_cond_beta_gap::State_::get( DictionaryDatum& d ) const
{
...
void
nest::hh_cond_beta_gap::State_::set( const DictionaryDatum& d )
{
...
nest::hh_cond_beta_gap::Buffers_::Buffers_( hh_cond_beta_gap& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::hh_cond_beta_gap::Buffers_::Buffers_( const Buffers_&,
  hh_cond_beta_gap& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
...
nest::hh_cond_beta_gap::hh_cond_beta_gap()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::hh_cond_beta_gap::hh_cond_beta_gap( const hh_cond_beta_gap& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::hh_cond_beta_gap::init_state_( const Node& proto )
{
  const hh_cond_beta_gap& pr = downcast< hh_cond_beta_gap >( proto );
  S_ = pr.S_;
}

void
nest::hh_cond_beta_gap::init_buffers_()
{
  ...
  B_.sys_.function = hh_cond_beta_gap_dynamics;
...
double
nest::hh_cond_beta_gap::get_normalisation_factor( double tau_rise,
  double tau_decay )
{
...
void
nest::hh_cond_beta_gap::calibrate()
{
...
bool
nest::hh_cond_beta_gap::update_( Time const& origin,
  const long from,
  const long to,
  const bool called_from_wfr_update )
{
  ...
  for ( long lag = from; lag < to; ++lag )
  {
    ...
    // B_.lag is needed by hh_cond_beta_gap_dynamics to
...
void
nest::hh_cond_beta_gap::handle( SpikeEvent& e )
{
...
void
nest::hh_cond_beta_gap::handle( CurrentEvent& e )
{
  assert( e.get_delay() > 0 );
...
void
nest::hh_cond_beta_gap::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}

void
nest::hh_cond_beta_gap::handle( GapJunctionEvent& e )
{
